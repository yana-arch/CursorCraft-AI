...
const SettingsModal: React.FC = () => {
    const { isSettingsOpen, setIsSettingsOpen, settings, saveSettings } = useEditor();
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="w-96 bg-gray-850 border border-gray-700 rounded-xl shadow-2xl overflow-hidden">
                            ...
                            {(['append', 'overwrite'] as AnimationMode[]).map((mode) => (
                                <button
                                    ...
                                    className={`py-2 px-3 rounded-lg text-xs font-medium border transition-all ${
                                        localSettings.defaultMode === mode ? 'bg-brand-600 border-brand-500 text-white shadow-lg' : 'bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-600'
                                    }`}
                                >
                ...
                <div className="px-6 py-4 bg-gray-800/30 border-t border-gray-700 flex justify-end">
                    <button onClick={handleSave} className="flex items-center space-x-2 bg-brand-600 hover:bg-brand-500 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-lg shadow-brand-500/20">
